...
class PerplexityBrowser:
    ...
    # ── Method 2: Ask AI query via /search ────────────────────────────

    async def ask(self, prompt: str, timeout: int = 45) -> str:
        """Send a prompt to Perplexity search and return the AI prose.
        ...
        Returns:
            The extracted prose text from Perplexity's AI answer.
        """
        await self._rate_limit_wait()
        page = await self._browser.new_page()
        ...
        try:
            ...
            url = f"{PERPLEXITY_SEARCH_URL}?q={query}"

            logger.info(f"[Browser] AI query: {prompt[:80]}...")
            logger.debug(f"[Browser] URL: {url[:120]}...")
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt < 2:
                        logger.warning(
                            f"[Browser] Nav attempt {attempt + 1} failed: {e}. Retrying..."
                        )
                        await asyncio.sleep(3)
                    else:
            ...
            logger.debug("[Browser] Waiting for AI response to generate...")
            try:
            ...
            logger.info(f"[Browser] AI response: {len(stable_text):,} chars")
            return stable_text.strip()

        except BrowserError:
            raise
        except Exception as e:
            logger.error(f"[Browser] AI query error: {e}")
            raise BrowserError(str(e))
        finally:
            await page.close()
